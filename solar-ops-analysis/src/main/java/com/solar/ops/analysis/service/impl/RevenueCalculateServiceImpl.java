...
public class RevenueCalculateServiceImpl implements RevenueCalculateService {
    ...
    public List<RevenueStatistics> calculateMonthlyRevenue(Long stationId, Integer year, Integer month) {
        log.info("开始计算电站[{}]在[{}年{}月]的月收益", stationId, year, month);

        YearMonth yearMonth = YearMonth.of(year, month);
        LocalDate monthStart = yearMonth.atDay(1);
        LocalDate monthEnd = yearMonth.atEndOfMonth();

        List<RevenueStatistics> dailyStats = new ArrayList<>();
        for (LocalDate date = monthStart; !date.isAfter(monthEnd); date = date.plusDays(1)) {
            RevenueStatistics daily = calculateDailyRevenue(stationId, date);
            if (daily != null) {
        ...
        if (CollectionUtils.isEmpty(dailyStats)) {
            log.warn("电站[{}]在[{}年{}月]没有日收益数据", stationId, year, month);
            return dailyStats;
        }
        ...
        dailyStats.add(monthlyStats);
        log.info("电站[{}]在[{}年{}月]的月收益计算完成，总收益：{}元", stationId, year, month, totalRevenue);
        return dailyStats;
    }
    ...
    public BigDecimal calculateUnitCost(Long stationId, BigDecimal energy) {
        ...
        BigDecimal operationCost = getStationDailyOperationCost(stationId);
    ...
    private BigDecimal getStationDailyOperationCost(Long stationId) {
